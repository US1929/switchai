...
export default function Premium() {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/verify', { headers: { 'x-auth-token': token } })
      .then(r => r.json())
      .then(d => { if (!d.valid) { sessionStorage.removeItem('switchai_token'); navigate('/login'); } else { setAuthChecked(true); } })
  ...
  const handleSubmit = async () => {
    ...
    const currentAnnualSpend = billAmount * freq;
  ...
  if (results) return <ResultsView results={results} form={form} onReset={() => { setResults(null); setStep(0); }} onLogout={() => { sessionStorage.removeItem('switchai_token'); navigate('/login'); }} />;
      ...
      <main style={styles.page}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.9rem', lineHeight: 1.6 }}>
            Accesso completo a tutte le offerte ARERA. Inserisci i tuoi dati di consumo e scopri quanto puoi risparmiare.
          </p>
        </div>
...
// ── RESULTS VIEW ─────────────────────────────────────────────
function ResultsView({ results, form, onReset, onLogout }) {
  ...
  return (
    <main style={styles.page}>
      ...
      <div style={styles.container}>
        ...
        {/* ── Nuovo calcolo + Logout ──────────────────── */}
        <div style={{ textAlign: 'center', display: 'flex', gap: 12, justifyContent: 'center' }}>
          <button className="btn btn-outline" onClick={onReset}>
            Nuovo calcolo
          </button>
          <button className="btn btn-outline" onClick={onLogout} style={{ opacity: 0.6 }}>
            Esci
          </button>
        </div>
